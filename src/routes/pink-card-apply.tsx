...
// // ── Per-user key — same logic as pink-card.tsx ────────────────────────────────
...
//   // Check if THIS user has already applied — show result directly
//   const savedResult = (() => {
//     try {
//       const raw = localStorage.getItem(getPinkCardKey());
//       return raw ? (JSON.parse(raw) as PinkCardResponse) : null;
...
//       try {
//         const res = await checkPinkCard(form.pan.trim().toUpperCase());
//         // Save result under THIS user's key
//         localStorage.setItem(getPinkCardKey(), JSON.stringify(res));
//         setResult(res);
//       } catch (err: unknown) {
//         setSubmitError(err instanceof Error ? err.message : "Submission failed. Please try again.");
...
//                 <XCircle className="mx-auto size-16 text-destructive" strokeWidth={1.5} />
//                 <h1 className="mt-5 font-display text-[34px] text-ink">Not Eligible</h1>
//                 <p className="mt-3 font-sans text-[14px] text-ink-muted">{result.reason_message}</p>
...
//                     <p className="font-sans text-[13px] text-ink-muted">
//                       Your annual income exceeds the ₹{result.threshold.toLocaleString("en-IN")}{" "}
//                       threshold for this scheme. You are ₹
//                       {Math.abs(result.gap).toLocaleString("en-IN")} above the limit.
//                     </p>
//                   )}
//                   {result.reason_code === "INELIGIBLE_NO_RECORD" && (
//                     <p className="font-sans text-[13px] text-ink-muted">
...
//           <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_280px] lg:items-start">
//             <div className="rounded-[18px] border border-rose/25 bg-black/45 p-6 sm:p-8">
//               {step === 1 && (
//                 <div className="space-y-5">
//                   <TextField
//                     label={t("apply.name")}
...
//   // Check if already applied — show result directly
//   const savedResult = (() => {
//     try {
...
//       try {
//         const res = await checkPinkCard(form.pan.trim().toUpperCase());
//         // Save result to localStorage so Check Status works
